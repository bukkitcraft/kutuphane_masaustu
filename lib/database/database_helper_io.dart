...
class DatabaseHelper {
  ...
  Future<Database> _initDB(String filePath) async {
    ...
    // Bazı eski kurulumlarda migration atlanabildiği için kritik kolonları garanti et.
    await _ensureCriticalColumns(db);
    return db;
  }

  Future<void> _ensureCriticalColumns(Database db) async {
    await _ensureColumnExists(
      db,
      tableName: 'checks',
      columnName: 'direction',
      alterSql: 'ALTER TABLE checks ADD COLUMN direction TEXT DEFAULT "Alınacak"',
    );
    await _ensureColumnExists(
      db,
      tableName: 'promissory_notes',
      columnName: 'direction',
      alterSql: 'ALTER TABLE promissory_notes ADD COLUMN direction TEXT DEFAULT "Alınacak"',
    );
  }
  ...
  Future<void> _ensureColumnExists(
    ...
    try {
      ...
      print('Warning: Could not ensure $tableName.$columnName column: $e');
    }
  }
