...
class EscrowScreen extends StatefulWidget {
  ...
  final bool openAddDialogOnLoad;
  final VoidCallback? onInitialDialogHandled;

  const EscrowScreen({
    super.key,
    this.initialBookId,
    this.openAddDialogOnLoad = false,
    this.onInitialDialogHandled,
  });
...
class _EscrowScreenState extends State<EscrowScreen> {
  ...
  bool _openedInitialAddDialog = false;
  ...
  void _openInitialAddDialogIfNeeded() {
    if (!mounted || _openedInitialAddDialog) return;
    if (!widget.openAddDialogOnLoad || widget.initialBookId == null) return;
    final initialBookId = widget.initialBookId;
    final hasBook = _books.any((b) => b.id == initialBookId);
    if (!hasBook) return;
    _openedInitialAddDialog = true;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        _showAddEditDialog(null, initialBookId);
        widget.onInitialDialogHandled?.call();
      }
    });
  }
  ...
  Future<int?> _pickMember(int? selectedMemberId) async {
                    ...
                    child: filteredMembers.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final isSelected = m.id == selectedMemberId;
                              return ListTile(
                                selected: isSelected,
                                leading: const Icon(Icons.person_outline_rounded),
                                title: Text(m.fullName),
                                subtitle: Text(m.memberNo),
  ...
  Future<int?> _pickBook(int? selectedBookId) async {
    final searchController = TextEditingController();
    final pickedId = await showDialog<int>(
      context: context,
      builder: (context) => StatefulBuilder(
        builder: (context, setStateDialog) {
          ...
          final filteredBooks = _books.where((b) {
            final canSelect = b.isAvailable || b.id == selectedBookId;
                ...
                children: [
                  ...
                  Expanded(
                    child: filteredBooks.isEmpty
                        ? const Center(child: Text('Kitap bulunamadi'))
                        : ListView.builder(
  ...
  void _showAddEditDialog([Escrow? escrow, int? initialBookId]) {
    ...
    int? selectedBookId = escrow?.bookId ?? initialBookId;
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  InkWell(
                    onTap: () async {
                      final picked = await _pickMember(selectedMemberId);
                      if (picked != null) {
                        setDialogState(() => selectedMemberId = picked);
                      }
                    },
                        ...
                        Expanded(
                          child: Text(
                            selectedMemberId != null
                                ? '${_members.firstWhere((m) => m.id == selectedMemberId).fullName} (${_members.firstWhere((m) => m.id == selectedMemberId).memberNo})'
                                : 'Uye sec (tikla ve ara)',
                            style: TextStyle(
                  ...
                  InkWell(
                    onTap: () async {
                      final picked = await _pickBook(selectedBookId);
                      if (picked != null) {
                        setDialogState(() => selectedBookId = picked);
                      }
                    },
                    borderRadius: BorderRadius.circular(12),
                    child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            selectedBookId != null
                                ? '${_books.firstWhere((b) => b.id == selectedBookId).title}${_books.firstWhere((b) => b.id == selectedBookId).authorName != null ? ' - ${_books.firstWhere((b) => b.id == selectedBookId).authorName}' : ''}'
                                : 'Kitap sec (tikla ve ara)',
                            style: TextStyle(
                              color: selectedBookId != null
                                  ? const Color(0xFF3E2723)
                                  : const Color(0xFF8D6E63),
                            ),
                            maxLines: 1,
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  image: _getImageProvider(selectedBook.coverImageUrl) != null
                                      ? DecorationImage(
                                          image: _getImageProvider(selectedBook.coverImageUrl)!,
                                          fit: BoxFit.cover,
                                          onError: (_, __) {},
                                        )
                                      : null,
                                ),
                                child: selectedBook.coverImageUrl == null ||
                                        selectedBook.coverImageUrl!.isEmpty
                                    ? const Icon(
                                        Icons.book_rounded,
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (selectedBook.authorName != null) ...[
                                      const SizedBox(height: 4),
                                      Text(
                                        'Yazar: ${selectedBook.authorName}',
                                        style: const TextStyle(
                                          fontSize: 13,
                                          color: Color(0xFF8D6E63),
                                        ),
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        image: _getImageProvider(escrow.bookCoverUrl) != null
                            ? DecorationImage(
                                image: _getImageProvider(escrow.bookCoverUrl)!,
                                fit: BoxFit.cover,
                                onError: (_, __) {},
                              )
                            : null,
