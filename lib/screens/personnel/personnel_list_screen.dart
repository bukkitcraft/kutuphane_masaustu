...
class _PersonnelListScreenState extends State<PersonnelListScreen> {
  ...
  void _showBulkSalaryPaymentDialog() {
    int? selectedDepartmentId;
    bool isProcessing = false;
    List<Personnel> selectedPersonnel = [];
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: DropdownButtonFormField<int>(
                      ...
                      items: _departments.map((d) {
                        return DropdownMenuItem(
                          value: d.id,
                          child: Text('${d.name} (${d.personnelCount} personel)'),
                        );
                      }).toList(),
                      onChanged: (value) {
                        setDialogState(() {
                          selectedDepartmentId = value;
                          // Seçilen departmandaki personelleri filtrele
                          selectedPersonnel = _personnelList
                              .where((p) => p.departmentId == value && p.isActive)
                              .toList();
                        });
                      },
                    ),
                  ...
                  Container(
                    ...
                    child: const Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Her personel için belirlenmiş maaş miktarı otomatik olarak kullanılacaktır.',
                            style: TextStyle(
                              color: Color(0xFF2E7D32),
                              fontSize: 13,
                            ),
                  ...
                  const SizedBox(height: 20),
                  // Seçilen Personeller Listesi
                  if (selectedDepartmentId != null) ...[
                    Text(
                      'Seçilen Departmandaki Personeller (${selectedPersonnel.length} kişi):',
                      style: const TextStyle(
                        fontWeight: FontWeight.bold,
                        color: Color(0xFF3E2723),
                        fontSize: 14,
                    ...
                    Container(
                      ...
                      child: selectedPersonnel.isEmpty
                          ? const Padding(
                              padding: EdgeInsets.all(20),
                              child: Center(
                                child: Text(
                                  'Bu departmanda aktif personel bulunamadı.',
                                  style: TextStyle(color: Color(0xFF8D6E63)),
                                ),
                              ),
                            )
                                  ...
                                  trailing: Column(
                                    ...
                                    children: [
                                      Text(
                                        ...
                                        style: TextStyle(
                                          fontWeight: FontWeight.bold,
                                          color: personnel.salary != null
                                              ? const Color(0xFF2E7D32)
                                              : Colors.orange,
                                        ),
                                      ),
                                      if (personnel.salary == null)
                                        const Text(
                                          '(Ödeme yapılmayacak)',
                                          style: TextStyle(
                                            fontSize: 10,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: isProcessing ||
                  ...
                  ? null
                  : () async {
                      // Maaş belirtilmiş personelleri filtrele
                      final personnelWithSalary = selectedPersonnel
                          .where((p) => p.salary != null && p.salary! > 0)
                          .toList();

                      if (personnelWithSalary.isEmpty) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Seçilen departmanda maaş belirtilmiş personel bulunamadı'),
                      ...
                      try {
                        ...
                        int successCount = 0;
                        int failCount = 0;
                        int skippedCount = selectedPersonnel.length - personnelWithSalary.length;

                        // Tüm giderleri bir kez al
                        final allExpenses = await _expenseService.getAll();
                        int expenseCounter = allExpenses.length;

                        // Her personel için ayrı gider kaydı oluştur (kendi maaşı ile)
                        for (final personnel in personnelWithSalary) {
                          try {
                            expenseCounter++;
                            // Gider numarası oluştur
                            final expenseNo =
                                'GID-${now.year}-${expenseCounter.toString().padLeft(4, '0')}';

                            final expense = Expense(
                              expenseNo: expenseNo,
                              title: 'Maaş Ödemesi - ${personnel.fullName}',
                              description:
                                  'Toplu maaş ödemesi - ${_departments.firstWhere((d) => d.id == selectedDepartmentId).name}',
                              amount: personnel.salary!,
                              category: 'Maaş',
                              expenseDate: now,
                              payeeName: personnel.fullName,
                              payeePhone: personnel.phone,
                              payeeEmail: personnel.email,
                              paymentMethod: 'Banka Transferi',
                              notes: 'Toplu maaş ödemesi',
                            );
                        ...
                        if (mounted) {
                          Navigator.pop(context);
                          String message = '$successCount personel için maaş ödemesi yapıldı';
                          if (skippedCount > 0) {
                            message += ' ($skippedCount personel maaş belirtilmediği için atlandı)';
                          }
                          if (failCount > 0) {
                            message += ' ($failCount hata)';
                          }
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 16),
                // Toplu Maaş Ödeme Button
                ElevatedButton.icon(
                  onPressed: () => _showBulkSalaryPaymentDialog(),
                  icon: const Icon(Icons.payments_rounded),
                  label: const Text('Toplu Maaş Ödeme'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFF2E7D32),
                    foregroundColor: Colors.white,
                    padding: const EdgeInsets.symmetric(
