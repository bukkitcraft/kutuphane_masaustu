...
class _MembersScreenState extends State<MembersScreen> {
  ...
  void _showDetailDialog(Member member) {
    final memberEscrowsFuture = member.id != null
        ? _escrowService.getByMember(member.id!)
        : Future.value(<Escrow>[]);
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      const Divider(),
                      const SizedBox(height: 8),
                      FutureBuilder<List<Escrow>>(
                        future: memberEscrowsFuture,
                        builder: (context, snapshot) {
                          ...
                          if (snapshot.hasError) {
                            return Container(
                              ...
                              child: const Text(
                                'Emanet bilgileri yuklenemedi',
                                style: TextStyle(color: Colors.red),
                              ),
                            );
                          }

                          final memberEscrows = snapshot.data ?? const <Escrow>[];
                          if (memberEscrows.isEmpty) {
                            return _buildDetailRow(
                              Icons.library_books_outlined,
                              'Emanet Kitaplar',
                              'Bu uyeye ait emanet kaydi bulunmuyor',
                            );
                          }
                          ...
                          return Column(
                            ...
                            children: [
                              _buildDetailRow(
                                Icons.library_books_outlined,
                                'Emanet Kitaplar',
                                '${memberEscrows.length} kayit',
                              ),
                              const SizedBox(height: 8),
                              ...memberEscrows.map((escrow) {
                                final isReturned = escrow.status == 'İade Edildi';
                                final isOverdue =
                                    escrow.status == 'Gecikmiş' || escrow.isOverdue;
                                final statusText = isReturned
                                    ? 'Iade edildi'
                                    : isOverdue
                                        ? 'Gecikmis'
                                        : '${escrow.remainingDays} gun kaldi';
                                ...
                                return Container(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 4),
                                      Text(
                                        'Odunc: ${escrow.formattedBorrowDate} • Iade: ${escrow.formattedDueDate}',
                                        style: TextStyle(
                                          fontSize: 12,
