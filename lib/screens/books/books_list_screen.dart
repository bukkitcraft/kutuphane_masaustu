...
class BooksListScreen extends StatefulWidget {
  final Future<void> Function(int? bookId)? onLendBook;

  const BooksListScreen({super.key, this.onLendBook});
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  image: selectedImageFile != null
                                      ...
                                      : imageUrlController.text.isNotEmpty
                                          ? DecorationImage(
                                              image: _getImageProvider(imageUrlController.text) != null
                                                  ? _getImageProvider(imageUrlController.text)!
                    ...
                    PopupMenuButton<String>(
                      ...
                      onSelected: (value) async {
                        ...
                        if (book.id == null) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('Kitap kimliği bulunamadı'),
                              backgroundColor: Colors.red,
                            ),
                          );
                          return;
                        ...
                        if (!book.isAvailable) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Bu kitap stokta olmadığı için emanet verilemez',
                              ),
                              backgroundColor: Colors.red,
                            ),
                          );
                        ...
                        if (widget.onLendBook != null) {
                          await widget.onLendBook!(book.id!);
                        }
                        _loadData();
                      },
                      itemBuilder: (context) => [
