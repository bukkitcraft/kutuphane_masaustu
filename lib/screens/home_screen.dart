...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  int? _pendingEscrowBookId;
  bool _openEscrowDialog = false;
  ...
  void _buildMenuItems() {
    ...
    final allMenuItems = [
      ...
      MenuItemData(title: 'VeriTabanı', icon: Icons.storage_rounded, color: const Color(0xFF6A1B9A), menuId: 'database'),
    ];
  ...
  Future<void> _openEscrowWithBook(int? bookId) async {
    final escrowIndex = _menuItems.indexWhere((item) => item.menuId == 'escrow');
    if (escrowIndex == -1) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Emanet menüsüne erişim izniniz yok'),
    ...
    _animationController.reset();
    setState(() {
      _pendingEscrowBookId = bookId;
      _openEscrowDialog = true;
      _selectedIndex = escrowIndex;
    });
    _animationController.forward();
  }

  void _onEscrowInitialDialogHandled() {
    if (!mounted) return;
    setState(() {
      _openEscrowDialog = false;
      _pendingEscrowBookId = null;
    });
  }
  ...
  Widget _getScreen(int index) {
    ...
    switch (selectedMenu.menuId) {
      ...
      case 'escrow':
        return EscrowScreen(
          initialBookId: _pendingEscrowBookId,
          openAddDialogOnLoad: _openEscrowDialog,
          onInitialDialogHandled: _onEscrowInitialDialogHandled,
        );
      case 'books':
        return BooksScreen(onLendBook: _openEscrowWithBook);
      case 'companies':
