...
class _FinanceScreenState extends State<FinanceScreen> with SingleTickerProviderStateMixin {
  ...
  String _incomeCategoryFilter = 'Tümü';
  DateTime? _incomeDateStart;
  DateTime? _incomeDateEnd;
  ...
  String _expenseCategoryFilter = 'Tümü';
  DateTime? _expenseDateStart;
  DateTime? _expenseDateEnd;
  ...
  String _generateUniqueDocumentNo(String prefix) {
    final now = DateTime.now();
    final timestamp =
        '${now.millisecondsSinceEpoch}${now.microsecond.toString().padLeft(3, '0')}';
    return '$prefix-${now.year}-$timestamp';
  }
  ...
  List<Income> get _filteredIncomes {
    ...
    // Kategori filtresi
    if (_incomeCategoryFilter != 'Tümü') {
      filtered = filtered.where((i) => i.category == _incomeCategoryFilter).toList();
    }
    
    // Tarih aralığı filtresi
    if (_incomeDateStart != null || _incomeDateEnd != null) {
      filtered = filtered.where((i) {
        if (_incomeDateStart != null && i.incomeDate.isBefore(_incomeDateStart!)) return false;
        if (_incomeDateEnd != null && i.incomeDate.isAfter(_incomeDateEnd!)) return false;
        return true;
      }).toList();
    }
  ...
  List<Expense> get _filteredExpenses {
    ...
    // Kategori filtresi
    if (_expenseCategoryFilter != 'Tümü') {
      filtered = filtered.where((e) => e.category == _expenseCategoryFilter).toList();
    }
    
    // Tarih aralığı filtresi
    if (_expenseDateStart != null || _expenseDateEnd != null) {
      filtered = filtered.where((e) {
        if (_expenseDateStart != null && e.expenseDate.isBefore(_expenseDateStart!)) return false;
        if (_expenseDateEnd != null && e.expenseDate.isAfter(_expenseDateEnd!)) return false;
        return true;
      }).toList();
    }
  ...
  List<Map<String, dynamic>> get _filteredCombinedDocuments {
    ...
    if (_documentNoSearchController.text.isNotEmpty) {
      ...
      filtered = filtered.where((doc) {
        final docNo = (doc['no'] as String).toLowerCase();
        final data = doc['data'];
        // Çek numarası veya senet numarası ile arama
        if (data is Check) {
          return docNo.contains(query) || data.checkNumber.toLowerCase().contains(query);
        } else if (data is PromissoryNote) {
    ...
    if (_detailedSearchController.text.isNotEmpty) {
      ...
      filtered = filtered.where((doc) {
        final company = (doc['company'] as String).toLowerCase();
        final docNo = (doc['no'] as String).toLowerCase();
        final description = (doc['description'] as String).toLowerCase();
        final data = doc['data'];
        
        // Çek numarası veya senet numarası ile arama
        bool matchesCheckNumber = false;
        if (data is Check) {
          matchesCheckNumber = data.checkNumber.toLowerCase().contains(query);
        }
      ...
      child: Column(
        children: [
          Padding(
            ...
            child: Column(
              children: [
                // İlk satır: Type Selector, Arama ve Ekle butonu
                Row(
                  children: [
                    // Type Selector
                    Container(
                ...
                Row(
                  children: [
                    ...
                    Container(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          value: _selectedTransactionType == 'Gelir' 
                              ? _incomeCategoryFilter 
                              : _expenseCategoryFilter,
                          items: (_selectedTransactionType == 'Gelir'
                              ? ['Tümü', 'Kitap Satışı', 'Üyelik Ücreti', 'Ceza', 'Diğer']
                              : ['Tümü', 'Kira', 'Maaş', 'Kırtasiye', 'Kitap Alımı', 'Bakım', 'Diğer']).map((c) {
                            return DropdownMenuItem(value: c, child: Text(c));
                          }).toList(),
                          onChanged: (value) => setState(() {
                            if (_selectedTransactionType == 'Gelir') {
                              _incomeCategoryFilter = value!;
                            } else {
                      ...
                      child: GestureDetector(
                        onTap: () async {
                          final date = await showDatePicker(
                            context: context,
                            initialDate: (_selectedTransactionType == 'Gelir' 
                                ? _incomeDateStart 
                                : _expenseDateStart) ?? DateTime.now(),
                          ...
                          if (date != null) {
                            setState(() {
                              if (_selectedTransactionType == 'Gelir') {
                                _incomeDateStart = date;
                              } else {
                                _expenseDateStart = date;
                              }
                            });
                          }
                        },
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  (_selectedTransactionType == 'Gelir' 
                                      ? _incomeDateStart 
                                      : _expenseDateStart) != null
                                      ? _formatDate((_selectedTransactionType == 'Gelir' 
                                          ? _incomeDateStart! 
                                          : _expenseDateStart!))
                                      : 'Başlangıç Tarihi',
                                  style: TextStyle(
                                    color: (_selectedTransactionType == 'Gelir' 
                                        ? _incomeDateStart 
                                        : _expenseDateStart) != null
                      ...
                      child: GestureDetector(
                        onTap: () async {
                          final date = await showDatePicker(
                            ...
                            initialDate: (_selectedTransactionType == 'Gelir' 
                                ? _incomeDateEnd 
                                : _expenseDateEnd) ?? DateTime.now(),
                            firstDate: (_selectedTransactionType == 'Gelir' 
                                ? _incomeDateStart 
                                : _expenseDateStart) ?? DateTime(2020),
                            lastDate: DateTime.now(),
                          );
                          if (date != null) {
                            setState(() {
                    ...
                    // Filtreleri Temizle
                    if ((_selectedTransactionType == 'Gelir' 
                        ? (_incomeCategoryFilter != 'Tümü' || _incomeDateStart != null || _incomeDateEnd != null)
                        : (_expenseCategoryFilter != 'Tümü' || _expenseDateStart != null || _expenseDateEnd != null)))
                      IconButton(
                        icon: const Icon(Icons.clear_all, color: Colors.red),
                        onPressed: () => setState(() {
                          if (_selectedTransactionType == 'Gelir') {
                            _incomeCategoryFilter = 'Tümü';
                            _incomeDateStart = null;
                            _incomeDateEnd = null;
                          } else {
                            _expenseCategoryFilter = 'Tümü';
                            _expenseDateStart = null;
                            _expenseDateEnd = null;
                          }
                        }),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    ElevatedButton.icon(
                      onPressed: () {
                        if (_documentTypeFilter == 'Çek') {
                          _showAddEditCheckDialog(null, _selectedCompany);
                        } else if (_documentTypeFilter == 'Senet') {
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  ListTile(
                                    ...
                                    onTap: () {
                                      Navigator.pop(sheetContext);
                                      _showAddEditPromissoryNoteDialog(
                                        null,
                                        _selectedCompany,
                                      );
                                    },
                    ...
                    if (_selectedDocument != null)
                      ElevatedButton.icon(
                        onPressed: () {
                          ...
                          if (type == 'Çek') {
                            _showAddEditCheckDialog(data as Check, _selectedCompany);
                          } else {
                            _showAddEditPromissoryNoteDialog(
                              data as PromissoryNote,
                              _selectedCompany,
                            );
                          }
                        },
  ...
  Future<void> _showAddEditCheckDialog([Check? check, Company? company]) async {
    ...
    // Yeni çek için otomatik numara oluştur
    String initialCheckNumber = '';
    if (!isEdit) {
      final nextNumber = await _checkService.getNextCheckNumber();
      initialCheckNumber = nextNumber.toString();
    } else {
      initialCheckNumber = check.checkNumber;
    }
    
    final checkNumberController = TextEditingController(text: initialCheckNumber);
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  TextField(
                    ...
                    readOnly: dialogSelectedPersonnelId != null,
                  ),
                  const SizedBox(height: 12),
                  Row(
                    children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'Çek Numarası *',
                      hintText: isEdit ? null : 'Otomatik oluşturulur',
                    ),
                    readOnly: !isEdit,
                  ),
                  const SizedBox(height: 12),
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final updatedCheck = Check(
                  id: check?.id,
                  checkNo: check?.checkNo ?? _generateUniqueDocumentNo('CEK'),
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final updatedNote = PromissoryNote(
                  id: note?.id,
                  noteNo: note?.noteNo ?? _generateUniqueDocumentNo('SNT'),
