...
class CheckService {
  ...
  Future<int> getNextCheckNumber() async {
    final db = await _dbHelper.database;
    final maps = await db.query('checks', columns: ['check_number']);
    ...
    int maxNumber = 0;
    for (var map in maps) {
      final checkNumberStr = map['check_number'] as String;
      final number = int.tryParse(checkNumberStr) ?? 0;
      if (number > maxNumber) {
        maxNumber = number;
      }
    }
